...
function renderOfficial(){
  ...
  $("officialCards").innerHTML = list.map(m => {
    ...
    return `<article class="card">
      <div class="cardHead">
        ...
        <div class="score ${statusCls(m)}">${m.lineupStatus.includes("Ufficial")?"OK":"WAIT"}</div>
      </div>
      <div class="grid">
        ...
        <div class="item"><span>Regola</span><strong>Solo nomi in rosa</strong></div>
      </div>
...
async function updateOfficialLineup(id){
  ...
  m.lineupStatus = "Ricerca ufficiali...";
  m.lastAutoUpdate = new Date().toLocaleString();
  render();

  try {
    const r = await fetch("/api/find-official-lineups", {
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({home:m.home, away:m.away, date:m.date, knownPlayers})
    });
    const out = await r.json();

    if(!out.ok){
      m.lineupStatus = "Ufficiali non trovate";
      m.source = out.source || "Auto lineup checker";
      m.lastAutoUpdate = new Date().toLocaleString();
      recalc(); render();
      return;
    }

    // Reset only the two teams involved
    state.players.filter(p=>p.team===m.home || p.team===m.away).forEach(p=>{
      p.status = "Panchina";
      p.source = p.source || "Squad DB";
    });

    let homeXI=0, awayXI=0, homeBench=0, awayBench=0;
    ...
    for(const item of out.players){
      ...
      p.source = out.source || "Official lineup";
    ...
    if(homeXI>=10 && awayXI>=10){
      m.lineupStatus = "Ufficiali";
    } else if(homeXI>=8 && awayXI>=8){
      m.lineupStatus = "Import parziale";
    } else {
      m.lineupStatus = "Ufficiali non affidabili";
      // if not enough, restore probable XI instead of damaging the model
      state.players.filter(p=>p.team===m.home || p.team===m.away).forEach(p=>{
    ...
    m.lastAutoUpdate = new Date().toLocaleString();
    m.source = out.source || "Official lineup";
    recalc(); render();
  } catch(e){
    m.lineupStatus = "Errore ufficiali";
    m.lastAutoUpdate = new Date().toLocaleString();
    recalc(); render();
  }
}
